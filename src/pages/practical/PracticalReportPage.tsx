...
import { ChevronLeft, ChevronRight, RefreshCw, User, Award, AlertTriangle, FileText, CheckSquare, XSquare } from 'lucide-react';
...
// 各文體格式核查規則（純函數，避免特殊字元問題）
function getFormatRules(genre: string): Array<{ label: string; deduction: string; check: (text: string) => boolean }> {
  switch (genre) {
    case 'speech':
      return [
        {
          label: '稱謂（開首頂格，含冒號，如：校長、各位老師、各位同學：）',
          deduction: '欠稱謂扣1分',
          check: (t) => /^[^\n]{0,30}[：:]/.test(t.trim()),
        },
        {
          label: '自我介紹（開首交代身份，如：大家好，我是……）',
          deduction: '欠自我介紹扣1分',
          check: (t) => /大家好|我是|本人是/.test(t.substring(0, 200)),
        },
        {
          label: '文末致謝（如：多謝各位。）',
          deduction: '欠文末致謝扣1分',
          check: (t) => /多謝各位|謝謝各位/.test(t.substring(t.length - 100)),
        },
        {
          label: '無書信格式（不含：鈞鑒、敬啟者、此致、祝頌語）',
          deduction: '添加多餘格式扣2分',
          check: (t) => !/鈞鑒|敬啟者|此致|祝頌|敬祝/.test(t),
        },
      ];
    case 'letter':
      return [
        {
          label: '上款／稱謂（開首頂格，末尾有冒號）',
          deduction: '欠上款扣1分',
          check: (t) => /^[^\n]{0,20}[：:]/.test(t.trim()),
        },
        {
          label: '祝頌語（含「祝」字，下一行寫祝福語）',
          deduction: '欠祝頌語扣1分',
          check: (t) => /祝/.test(t),
        },
        {
          label: '署名（含啟告語，如：謹啟、敬啟）',
          deduction: '欠署名扣1分',
          check: (t) => /謹啟|敬啟|拜啟|謹上/.test(t),
        },
        {
          label: '日期（含年月日）',
          deduction: '欠日期扣1分',
          check: (t) => /年.{0,5}月.{0,5}[日號]/.test(t),
        },
        {
          label: '無演講辭格式（不含：大家好，我是……開首）',
          deduction: '添加多餘格式扣2分',
          check: (t) => !/^大家好/.test(t.trim()),
        },
      ];
    case 'proposal':
      return [
        ...
        },
        {
          label: '標題（含「建議」二字）',
          deduction: '欠標題扣1分',
          check: (t) => /建議/.test(t.substring(0, 150)),
        },
        {
          label: '署名（含謹啟）',
          deduction: '欠署名扣1分',
          check: (t) => /謹啟|敬啟/.test(t),
        },
        {
          label: '日期（含年月日）',
          deduction: '欠日期扣1分',
        ...
        {
          label: '無祝頌語（建議書不應有祝頌語）',
          deduction: '添加多餘格式扣2分',
          check: (t) => !/祝頌|敬祝|順頌/.test(t),
        },
      ];
    case 'report':
      return [
        ...
        },
        {
          label: '標題（含「報告」二字）',
          deduction: '欠標題扣1分',
          check: (t) => /報告/.test(t.substring(0, 150)),
        },
        {
          label: '署名（含謹啟）',
          deduction: '欠署名扣1分',
        ...
        {
          label: '無多餘格式（不含：多謝各位、祝頌語、專此）',
          deduction: '添加多餘格式扣2分',
          check: (t) => !/多謝各位|祝頌|專此/.test(t),
        },
      ];
    case 'commentary':
    case 'article':
      return [
        {
          label: '標題（文章頂部，字數少於30字）',
          deduction: '欠標題扣1分',
          check: (t) => {
            const firstLine = t.trim().split('\n')[0].trim();
            return firstLine.length > 0 && firstLine.length < 30;
          },
        },
        {
          label: '署名（文末，不含「啟」字）',
          deduction: '欠署名扣1分',
          check: (t) => {
            const last150 = t.substring(t.length - 150).trim();
            return last150.length > 0;
          },
        },
        {
          label: '無書信格式（不含上款冒號、祝頌語）',
          deduction: '添加多餘格式扣2分',
          check: (t) => !/祝頌|鈞鑒|敬啟者/.test(t),
        },
      ];
    default:
      return [];
...
function parseEssayToHtml(text: string): string {
  ...
  return lines.map((line, idx) => {
    ...
    const isDate = isInLastSection && /年.*月.*[日號]/.test(trimmed);

    if (isTitle) return `<div style="text-align:center;font-weight:bold;margin:0.6em 0">${processed}</div>`;
...
export function PracticalReportPage({ onNext, onPrev }: PracticalReportPageProps) {
  ...
  const formatCheckResults = useMemo(() => {
    ...
    const rules = getFormatRules(practicalGenre);
  ...
  const generateMockReport = (work: typeof currentWork): PracticalReport => {
    ...
    return {
      studentWork: work!,
      grading, contentScore, organizationScore,
  ...
  const handleDownloadReport = () => {
    ...
    const feedbackSections = [
      ...
      `<h3>${name} (${score}/${max})</h3>
       <div class="strengths"><b>優點：</b><ul>${fb.strengths.map((s: string) => `<li>${s}</li>`).join('')}</ul></div>
       <div class="improvements"><b>改善：</b><ul>${fb.improvements.map((s: string) => `<li>${s}</li>`).join('')}</ul></div>`
    ).join('');
...
h1{text-align:center;color:#B5726E}
h2{border-bottom:2px solid #B5726E;padding-bottom:5px;color:#B5726E;margin-top:30px}
              ...
              <CardContent className="space-y-2 pt-0">
                {formatCheckResults.map((result, i) => (
                  <div key={i} className={`flex items-start gap-2 text-xs p-2 rounded ${result.passed ? 'bg-green-50' : 'bg-orange-50'}`}>
                    ...
                    <div>
                      <div className={result.passed ? 'text-green-700' : 'text-orange-700'}>
                    ...
                    <div key={item.title} className="border-t pt-4">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <ul className="list-disc list-inside">{item.feedback.strengths.map((s, i) => <li key={i}>{s}</li>)}</ul>
                        </div>
                        <div>
                          <p className="text-orange-600 mb-1">改善：</p>
                          <ul className="list-disc list-inside">{item.feedback.improvements.map((s, i) => <li key={i}>{s}</li>)}</ul>
                        </div>
                      </div>
                    </div>
                  ))}
      ...
      <div className="fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-[#E2E8F0] flex items-center justify-between px-6 z-40">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={handleDownloadReport} className="gap-2">
            <FileText className="w-4 h-4" />下載 HTML
          </Button>
